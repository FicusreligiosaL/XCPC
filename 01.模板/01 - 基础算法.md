...
#define rep(x,y,z) for(int x=(y),_end=(z),_step=((y)<=(_end)?1:-1);(y)<=(_end)?x<=(_end):x>=(_end);x+=_step)
typedef pair<int,int> pii;typedef pair<ll,ll> pll;typedef vector<pair<int,int>> vpii;
template<typename tp1,typename tp2>void chmax(tp1 &x,const tp2 &y){if(x<y)x=y;}
template<typename tp1,typename tp2>void chmin(tp1 &x,const tp2 &y){if(x>y)x=y;}
template<class... Args>void _(Args... args){auto _=[&](auto x){cout<<x<<" ";};cout<<"--->";int arr[]={(_(args),0)...};cout<<'\n';}
const int inf=0x3f3f3f3f;
const int mod=998244353;
const int N=2e5+10;
int ksm(int a,int b,int p=mod){int res=1;while(b){if(b&1)res=1ll*res*a%p;a=1ll*a*a%p;b>>=1;}return res;}
