...
- 吸收公式：$\displaystyle k\binom{n}{k}=n\binom{n-1}{k-1} $ 
- 吸收公式扩大版：$\displaystyle\binom{n}{k}\binom{k}{m}=\binom{n}{m}\binom{n-m}{k-m}$ 
- 帕斯卡恒等式：$\displaystyle\binom{n}{k}=\binom{n-1}{k-1}+\binom{n-1}{k}$ 
- 求和公式1：$\displaystyle\sum_{k = 0}^{n}\binom{n}{k} = \binom{n}{0}+\binom{n}{1}+\cdots+\binom{n}{n}=2^n$ 
- 求和公式2（曲棍球棒恒等式）$\displaystyle\sum_{i=r}^{n}\binom{i}{r} = \binom{r}{r}+\binom{r+1}{r}+\cdots+\binom{n}{r}=\binom{n+1}{r+1}$ 
- 范德蒙德卷积公式：$\displaystyle\sum_{j=0}^{k}\binom{m}{j}\binom{n}{k-j} = \binom{m+n}{k}$
...
- $\displaystyle \sum_{i=1}^{n}i^2{n\choose i}=n*(n+1)*2^{n-2}$ （二项式定理积分证明或者简单组合讨论或者吸收公式）
- $\displaystyle \sum_{i=1}^{n}\dfrac{1}{i}{n\choose i}=\sum_{i=1}^{n}\dfrac{2^i-1}{i}$ （二项式定理使用积分证明）
- $\displaystyle \sum_{i=0}^{n}{n\choose i}^2={2n\choose n}$ （范德蒙德卷积公式）
